...
self.addEventListener("push", event => {
  const payload = event.data ? event.data.json() : { title: "Luma", body: "You have a new update.", url: "/notifications" };
  event.waitUntil(self.registration.showNotification(payload.title || "Luma", { body: payload.body || "You have a new update.", icon: "/favicon.svg", data: { url: payload.url || "/notifications" } }));
});
