...
function rateLimit(req: express.Request, res: express.Response, next: express.NextFunction) {
  ...
  const now = Date.now(); const ip = req.ip || req.socket.remoteAddress || "unknown"; const strict = req.path.includes("uploads") || req.path.includes("oauth"); const max = strict ? 20 : 180; const key = `${strict ? "strict" : "standard"}:${ip}`; const record = requestWindows.get(key);
