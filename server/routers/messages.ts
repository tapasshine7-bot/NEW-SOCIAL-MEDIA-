...
export const messagesRouter = router({
  ...
  typing: protectedProcedure.input(z.object({ conversationId, active: z.boolean() })).mutation(async ({ ctx, input }) => { const db = await requireDb(); const current = await membership(db, input.conversationId, ctx.user.id); if (!input.active) { await db.delete(conversationTyping).where(and(eq(conversationTyping.conversationId, current.conversation.id), eq(conversationTyping.userId, ctx.user.id))); return { success: true } as const; } await db.insert(conversationTyping).values({ conversationId: current.conversation.id, userId: ctx.user.id, expiresAt: new Date(Date.now() + 8_000) }).onDuplicateKeyUpdate({ set: { expiresAt: new Date(Date.now() + 8_000) } }); return { success: true } as const; }),
