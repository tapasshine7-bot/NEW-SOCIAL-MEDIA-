...
describe("local account workflows", () => {
  ...
  it("registers a member identity and issues their protected session cookie", async () => {
    ...
    const run = context(); const result = await localAuthRouter.createCaller(run.ctx).register({ displayName: "New Member", email: "member@example.com", password: "securepass123" });
  ...
  it("rejects a duplicate email before storing a password", async () => {
    ...
    await expect(localAuthRouter.createCaller(context().ctx).register({ displayName: "Member", email: "member@example.com", password: "securepass123" })).rejects.toMatchObject({ code: "CONFLICT" });
  });
  it("does not issue a session for an invalid password", async () => {
    const hash = await hashPassword("securepass123"); const user = { id: 7, openId: "local_member", name: "Member", accountStatus: "active" };
  ...
  it("signs in a valid local member and issues the same protected session cookie", async () => {
    const hash = await hashPassword("securepass123"); const user = { id: 7, openId: "local_member", name: "Member", accountStatus: "active" }; const lastSignedInSet = vi.fn(() => ({ where: vi.fn() }));
    ...
    const run = context(); await expect(localAuthRouter.createCaller(run.ctx).signIn({ email: "member@example.com", password: "securepass123" })).resolves.toMatchObject({ success: true, user: { openId: "local_member" } }); expect(lastSignedInSet).toHaveBeenCalled(); expect(run.cookies).toEqual([{ name: COOKIE_NAME, value: "signed-local-session" }]);
  });
});
